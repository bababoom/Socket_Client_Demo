...
public class JIMHistoryTextPane extends JIMSendTextPane {
	...
	public JIMHistoryTextPane() {
		...
		messageConcurrentLinkedQueue = new ConcurrentLinkedQueue<Message>() ;
		/*messageConcurrentLinkedQueue.add(new Message(true,"hello world")) ;
		messageConcurrentLinkedQueue.add(new Message(false,"hello world")) ;
		messageConcurrentLinkedQueue.add(new Message(true,"hello world")) ;
		messageConcurrentLinkedQueue.add(new Message(false,"hello world")) ;
		messageConcurrentLinkedQueue.add(new Message(true,"hello world")) ;
		messageConcurrentLinkedQueue.add(new Message(false,"hello world")) ;
...
//				Image image = new ImageIcon(this.getClass().getResource("/com/test/image/下载.jpg")).getImage();
				...
				if (point != null) {
					...
					if (message.isSelf()) {
						 ...
						 Image image2 = getHeadImageIcon("image/头像背景@2x(1).png", 40, 40).getImage();
...
//					topGap += messageHeight + 20;
	...
	public ConcurrentLinkedQueue<Message> getMessageConcurrentLinkedQueue() {
		return this.messageConcurrentLinkedQueue ;
	}
	...
	public static ImageIcon getHeadImageIcon(String head, int width, int height) {
		...
		if (head == null || head.equals("") || head.equals("null")) {
			headImageIcon = new ImageIcon("image/下载.png");//默认加载头像
		} else {
			try {
